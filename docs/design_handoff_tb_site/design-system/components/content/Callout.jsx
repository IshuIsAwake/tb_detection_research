...
/**
 * Callout — the intellectual-honesty motif as UI. Kinds:
 *  note (info)    — neutral aside
 *  decision (good)— a settled decision / lever closed
 *  landmine (warn)— a gotcha that costs a debugging session
 *  retraction (bad)— a corrected wrong read (use <s> in children)
 * A left accent bar + tinted ground + small label.
 */
export function Callout({ kind = "note", title, children, style = {} }) {
  const conf = {
