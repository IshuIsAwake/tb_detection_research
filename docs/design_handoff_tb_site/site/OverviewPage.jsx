const { Button, Badge, Tag } = window.TBDetectionResearchDesignSystem_ccf532;

function GoalCard({ n, title, body, status }) {
  return (
...
function PaperSection({ label, title, children }) {
  return (
...
function Para({ children }) {
  return (
    <p style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-body)", color: "var(--ink-2)", lineHeight: "var(--lh-body)", margin: "0 auto var(--sp-3)", maxWidth: "62ch", textWrap: "pretty" }}>
      {children}
    </p>
  );
}
  ...
  return (
    <main style={{ maxWidth: "820px", margin: "0 auto", padding: "0 var(--gutter) var(--sp-9)", textAlign: "center" }}>
      ...
      <header style={{ padding: "var(--sp-9) 0 var(--sp-6)" }}>
        ...
        <p style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-lead)", color: "var(--ink-2)", lineHeight: "var(--lh-body)", margin: "1.25rem auto 0", maxWidth: "60ch", textWrap: "pretty" }}>
        ...
        <div style={{ display: "flex", gap: "0.75rem", marginTop: "var(--sp-6)", flexWrap: "wrap", justifyContent: "center" }}>
          <Button variant="primary" onClick={() => setPage("results")} iconRight={<window.Icon name="arrow-right" size="1em" />}>Read results &amp; ablation</Button>
          <Button variant="secondary" onClick={() => setPage("data")}>Dataset research</Button>
          <Button variant="ghost" onClick={() => setPage("references")}>References</Button>
        </div>
      </header>
      ...
      <PaperSection label="03 · Method" title="Protocol">
        ...
        <div style={{ display: "inline-flex", gap: "0.5rem", flexWrap: "wrap", justifyContent: "center", marginTop: "var(--sp-2)" }}>
          ...
          <Tag>multi-seed</Tag>
          <Tag tone="primary">mosaic @ 512, batch 16</Tag>
        </div>
      </PaperSection>
    </main>
  );
