...
import { PaperRef, Callout } from "@/design-system";
import { Page, PageHeader, SectionTitle, Icon } from "@/components/Shell";
import { PAPER_GROUPS } from "@/data/content";

export function ReferencesPage() {
  const [readState, setReadState] = React.useState<Record<string, boolean>>(() => {
    const m: Record<string, boolean> = {};
    PAPER_GROUPS.forEach((g) => g.papers.forEach((p) => { m[p.title] = !!p.read; }));
    return m;
  });
  ...
  return (
    <Page>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "var(--sp-7)", padding: "1rem 1.2rem", background: "var(--primary-tint)", border: "1px solid var(--primary-tint-2)", borderRadius: "var(--r)" }}>
        <Icon name="book-open" color="var(--primary)" size="1.2rem" />
        ...
        <div style={{ flex: 1, height: "6px", background: "color-mix(in srgb, var(--primary) 18%, transparent)", borderRadius: "var(--r-pill)", overflow: "hidden", maxWidth: "260px" }}>
          <div style={{ width: (done / total * 100) + "%", height: "100%", background: "var(--primary)", transition: "width 320ms var(--ease)" }} />
        </div>
      </div>

      {PAPER_GROUPS.map((g, gi) => (
        <section key={gi} style={{ marginBottom: "var(--sp-7)" }}>
          <SectionTitle kicker={`Section ${gi + 1}`}>{g.group}</SectionTitle>
