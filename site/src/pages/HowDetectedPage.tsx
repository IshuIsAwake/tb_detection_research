...
const SOURCES: Source[] = [
  ...
  { label: "WHO — Chest radiography & CAD screening guidelines", url: "https://www.who.int/publications/i/item/9789240022676" },
...
function ReadMore() {
  ...
  return (
    <button
      ...
      style={{ all: "unset", boxSizing: "border-box", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem", width: "100%", marginTop: "var(--sp-5)", padding: "var(--sp-5)", border: "1px solid var(--primary-tint-2)", background: "var(--primary-tint)", borderRadius: "var(--r)", transition: "box-shadow var(--transition)" }}
      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.boxShadow = "var(--shadow)"; }}
      onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.boxShadow = "none"; }}
    >
      <span>
        <span style={{ display: "block", fontFamily: "var(--font-mono)", fontSize: "var(--text-label)", textTransform: "uppercase", letterSpacing: "var(--ls-label)", color: "var(--primary)", fontWeight: "var(--w-semibold)", marginBottom: "0.25rem" }}>Read more</span>
        <span style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-body)", fontWeight: "var(--w-semibold)", color: "var(--ink)" }}>How is active TB detected — the four methods compared</span>
      </span>
  ...
  return (
    <main style={{ maxWidth: "var(--content-max)", margin: "0 auto", padding: "var(--sp-7) var(--gutter) var(--sp-9)" }}>
      ...
      <header style={{ marginBottom: "var(--sp-6)" }}>
        ...
        <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "var(--text-display)", fontWeight: "var(--w-semibold)", letterSpacing: "var(--ls-tight)", lineHeight: "var(--lh-tight)", margin: "0.5rem 0 0" }}>How is TB detected?</h1>
      ...
      <ReadSection n={1} title="Why don't we treat everyone?">
        <Prose>
          Roughly 2 billion people carry latent TB, but only 5–10% ever progress to active disease. Treating the
          entire infected population would mean giving drugs to about 1.8 billion people who would never have
          fallen ill — and that does real harm.
        </Prose>
        <CardGrid cols={2}>
          <InfoCard title="Hepatotoxicity (liver damage)">
            <Bullets items={[
              <>The first-line drugs for latent TB (isoniazid, rifampin) are harsh and carry a real risk of drug-induced liver injury.<Cite n={1} /></>,
        ...
        <Callout kind="note" title="Who does get treated" style={{ marginTop: "var(--sp-4)" }}>
          Because of these risks, the WHO and CDC recommend treating latent TB only in high-risk groups —
          people whose immune systems are compromised (such as those living with HIV or organ-transplant
          recipients), or anyone recently in close contact with an active TB patient.<Cite n={2} />
        </Callout>
      </ReadSection>

      <ReadSection n={2} title="Detecting latent TB">
        <Prose>
          Latent TB hides — there are no symptoms and no bacteria in the sputum to find. Instead, both tests look
          for the immune system's <em>memory</em> of having fought TB.
        </Prose>
        <CardGrid cols={2}>
          <InfoCard title="IGRA (interferon-gamma release assay)">
            <Bullets items={[
              ...
              <>High specificity — it does not trigger false positives from a previous BCG (TB) vaccination.<Cite n={2} /></>,
            ]} />
          </InfoCard>
          <InfoCard title="TST (tuberculin skin test / Mantoux)">
            <Bullets items={[
              ...
              <>Cheaper but less accurate than IGRA — it often reads false-positive in people given the BCG vaccine as children.<Cite n={4} /></>,
            ]} />
          </InfoCard>
        </CardGrid>
      </ReadSection>

      <ReadSection n={3} title="Detecting active TB">
        <Prose>
          Active TB can be found directly, because the bacteria are multiplying and shedding into the lungs and
          sputum. The methods trade off cost, speed, and certainty.
        </Prose>
        <CardGrid cols={2}>
          <InfoCard title="Rapid molecular assays (e.g. GeneXpert)">
            <Bullets items={[
              ...
              <>Crucially, it detects Rifampicin-resistance mutations at the same time — in under two hours.<Cite n={3} /></>,
            ]} />
          </InfoCard>
          ...
          <InfoCard title="Radiology (chest X-ray)">
            <Bullets items={[
              "Reveals the structural damage, cavities, and fluid build-up caused by active infection.",
              <>Fast and cheap enough to run at scale, which makes it the frontline screening filter — increasingly read by AI/CAD software.<Cite n={[5, 6]} /></>,
            ]} />
          </InfoCard>
        </CardGrid>
        <Prose style={{ marginTop: "var(--sp-5)" }}>
          Chest X-rays are the perfect frontline filter precisely because they are fast and inexpensive. If every
          person with a bad cough needed a $15–$20 GeneXpert test, high-burden health systems would collapse under
          the cost and backlog. Highly accurate AI and CAD (computer-aided detection) software lets a clinic screen
          thousands of X-rays quickly — safely sending home the clear lungs and reserving the expensive lab tests
          for the images flagged as high-risk.<Cite n={5} />
        </Prose>
        <ReadMore />
      </ReadSection>
