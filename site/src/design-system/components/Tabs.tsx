...
export function Tabs({ items = [], value, defaultValue, onChange, style = {} }: TabsProps) {
  ...
  return (
    <div role="tablist" style={{ display: "inline-flex", gap: "0.2rem", padding: "0.25rem", background: "var(--paper-2)", border: "1px solid var(--line)", borderRadius: "var(--r)", ...style }}>
      {items.map((it) => {
        ...
        return (
          <button
            ...
            onMouseEnter={(e) => { if (!on) (e.currentTarget as HTMLElement).style.background = "var(--surface)"; }}
